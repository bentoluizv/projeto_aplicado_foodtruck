...
class MigrationService(BaseService):
    ...
    def execute_operation(self, operation: str, **kwargs) -> Any:  # noqa: PLR0911
    ...
    def _init_database(self, **kwargs) -> MigrationResult:
        """Initialize database with migrations.

        Returns:
            MigrationResult containing operation result
        """
    ...
    def _current_migration(self) -> CurrentMigrationResult:
        """Get current migration revision.

        Returns:
            CurrentMigrationResult containing current revision info
        """
        return_code, stdout, stderr = self._run_alembic_command([
            'alembic',
            'current',
    ...
    def _migration_history(
        ...
        Returns:
            MigrationHistoryResult containing migration history
        """
        ...
        if return_code == 0:
            history = (
                stdout.strip() if stdout.strip() else 'No migration history'
            )
            return MigrationHistoryResult(
                success=True,
    ...
    def _create_migration(self, message: str, **kwargs) -> MigrationResult:
        ...
        if return_code == 0:
            return MigrationResult(
                ...
                details=(
                    stdout.strip() if stdout.strip() else 'Migration created'
                ),
            )
        else:
            return MigrationResult(
    ...
    def _reset_database(
        ...
        if not force:
            return MigrationResult(
                ...
                error=(
                    'This is a destructive operation. Use --force to confirm.'
                ),
            )
        ...
        if upgrade_result.success:
            return MigrationResult(
                ...
                details=(
                    'Database downgraded to base and upgraded to head'
                ),
            )
        else:
            return upgrade_result

    def _database_status(self) -> MigrationStatus:
        """Get database status and migration info.

        Returns:
            MigrationStatus containing database status
        """
        # Get current migration
        current_result = self._current_migration()

        # Check if database is accessible
        connection_ok = self.database_service.test_connection()

        return MigrationStatus(
            success=True,
            message='Database status retrieved',
            connection='OK' if connection_ok else 'FAILED',
            current_migration=current_result.current or 'Unknown',
